...
func main() {
	// read stuff from .env
	dotenvpath, customPath := os.LookupEnv("FORMENVPATH")
	var dotenverr error
	if customPath {
	...
	serverhost := os.Getenv("formhost")

	// create database handle
	var dsnerr error
	config.DB, dsnerr = sql.Open("mysql", dbuser+":"+dbkey+"@tcp("+dbloc+":"+dbport+")/secretform?parseTime=true")
	if dsnerr != nil {
